...
| 2026-05-29 | `momentum-directional-precheck` (result) | **MARGINAL(배포불가)** — 저빈도 모멘텀 TSMOM(LS/LO)+XSMOM, 56테스트. 2/56 PASS(ETH-TSMOM-LS L2만, BH·OOS 충족)이나 강건성 전멸: BTC·EW포트폴리오·long-only 전부 BH 탈락, lookback 스파이크(L1 Sh~0→L2 ~1.0), XSMOM IS 1.13→OOS 0.21 과적합, MDD −67~76%. 사전지정 majors+포트폴리오가 확증 못 함 → 이식불가 단일자산 아티팩트. 단 모멘텀의 2022 베어 회피(MDD −45~67% vs buy&hold −81%)는 실재 → 리스크오버레이 단서. `src/momentum/` |
| 2026-05-29 | `eth-momentum-walkforward` (result) | **아티팩트 미확정(예상 빗나감), 단 엣지=2022 크래시 포착** — precheck 유일통과 ETH-TSMOM-LS L2 walk-forward(anchored train2yr/test6mo, 13fold). A고정·B적응형 모두 stitched OOS Sharpe 0.91 > buy&hold 0.44, 적응형이 13fold 전부 L=2 선택(견고). 사전등록 생존기준 충족 → 단일구간 아티팩트 단정 틀림. 그러나 OOS 우위 거의 전부가 2022 베어 숏 포착(상위2fold=63% PnL, 평시 buy&hold 동급, MDD −67%) → 방향성 알파 아니라 레짐(크래시)-의존 리스크오버레이. 자본투입 대상 아님. `src/momentum/walkforward.py` |
| 2026-05-29 | `crisis-alpha-generality` (result) | **REJECTED-with-caveats — 크래시 포착은 크립토 일반성질 아님** — 8자산 walk-forward 확장+크래시레짐 분해. 사전등록 3게이트 전부 미달(크래시집중 4/8, 에피소드 2/5, 평시드래그 2/8), DD임계 0.20~0.40 전부 강건. 핵심: 크래시 형태 의존(LUNA 완만→5/6 포착 vs FTX 급충격→7/8 휩쏘). ETH는 평시 PnL(+264%)>크래시(+232%)=크래시 컨벡시티 아님, 추세+이상치 에피소드. **적대적검증 workflow가 EW inner-join 버그 발견→정정(EW outer Sharpe 0.58>BH 0.45 진단용, 게이트엔 무관)**. 이식가능 알파 미확인, 진짜 보편성은 비-크립토(주식/채권/상품) 필요. `src/momentum/crisis_alpha.py` |
| 2026-06-08 | `onchain-netflow-precheck` (design+result) | **FAIL, 폐기 — 온체인도 방향성 알파 없음** — "유료 API 수익 가능?"→ 무료 공개API와 정보차원 다른 유일후보=온체인. **Coin Metrics Community(무료, 키불필요)** BTC 15.1yr/ETH 10.8yr 거래소 netflow. 사전등록 게이트(경제성/boot+BH/레짐/IS-OOS/대칭성/EW), z{30,90}×LAG{1,2}=12변형. **0/12 PASS** — 경제성서 결정적 킬(보유기간당 edge −15~+1.8bps ≪ 15bps). 레그비대칭이 진단: 롱+7~14bps(세속강세장 베타일 뿐)/숏−19~−32bps(상승에 숏=손실)→방향성정보 0, net −67~−99% vs BH +수만%. robustness 전부 동일 FAIL. **무료서 edge부재 확인=일봉 netflow 유료결제 비추천.** 단 인트라데이/엔티티레벨 플로우·비방향성(리스크오버레이) 용도는 미배제. `src/onchain/` |
| 2026-06-08 | `onchain-netflow-overlay` (design+result) | **FAIL, 폐기 — 비방향성 오버레이도 vol에 지배됨** — directional FAIL 후 (c)비방향성 피벗. netflow를 방향 아니라 노출조절에(유입급증→플랫). **anti-fooling 삼중차단**: B1 변동성관리(빈도매칭)·B2 랜덤null(부트스트랩)·레짐(크래시제거). BTC/ETH/EW 전부 FAIL, **게이트2(정보)서 킬**: BTC는 랜덤 이김(p=0.03)이나 **vol 오버레이에 짐(Sharpe 0.91<0.99)**=netflow 하방정보를 실현변동성이 이미 더 잘 포착, ETH/EW는 랜덤조차 못이김(p=0.27/0.08). 개선폭 미미(+0.03~0.09)+임계취약(THR0.5서 음). 게이트3은 통과(크래시집중 아님)지만 무의미. **B1/B2 없었으면 BTC false-positive로 보였을 것 — 정직검증 설계가 차단.** 일봉 netflow=방향성도 오버레이도 무료 가격데이터 못넘음. 미배제: 인트라데이/엔티티 플로우, MVRV바스켓. `src/onchain/overlay.py` |
| 2026-06-08 | `onchain-basket-precheck` (접근2 result) | **FAIL, 폐기 — 온체인 라인 종료** — netflow 외 메트릭(MVRV·활성주소·Tx수) directional. precheck.py 게이트 재사용, 36테스트(3메트릭×{BTC,ETH,EW}×z{30,90}×LAG{1,2}) BH보정. 전 메트릭 FAIL. **MVRV=베타지배 극단형(고평가→숏 −278~−384bps, 불장에 숏=자살, SHORT레그 −37~−44)**, **활성주소=무edge(+0.6~3.2bps≪15)**, **TxCnt|ETH만 경제성통과(+18.5bps,net+484%)이나 BTC반대부호·bootP0.348·BH탈락·OOS불안정=단일자산 아티팩트**(교차자산+BH+포트 게이트가 정확히 가려냄). **일봉 무료 온체인 3종(netflow directional/overlay/바스켓) 전패 → 일봉 온체인 유료결제 비추천 확정.** 미배제: 인트라데이/엔티티(고래) 플로우만(burden 극히 높음). `src/onchain/basket.py` |
