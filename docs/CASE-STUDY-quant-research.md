...
**적용 직무**: 퀀트 리서치, 데이터 사이언스, ML/리서치 엔지니어링 —
"가설을 세우고, 자기 아이디어를 증거로 죽이고, 음성 결과를 규율 있게
문서화"가 핵심인 모든 역할.

**엘리베이터 피치(재사용 가능)**:
> "암호화폐 자동매매 리서치에서 10개 가설을 사전 폐기기준과 데이터-우선
> 검증으로 체계적으로 반증했습니다. 핵심은 알파를 찾은 게 아니라,
> *자본을 걸기 전에 나쁜 아이디어를 싸게 죽이는 방법론*입니다 — 부분
> 데이터의 +275bps 신호가 과적합임을 전체표본으로 잡아내 실거래 실패를
> 막은 사례, gross 수익이 비용 후 사라짐을 비용분해로 드러낸 사례,
> 비결정적 LLM을 재현 가능하게 엔지니어링한 사례가 문서로 남아 있습니다.
> 음성 결과 10건 전부 추적 가능하게 기록되어 있습니다."

**면접에서 보여줄 단일 사례(추천)**: #10 — "부분 데이터에서 매력적인
신호를 봤지만 사전 폐기기준과 전체표본 규율로 과적합임을 밝혀 폐기한"
이야기. 자기 아이디어를 죽이는 규율 + 통계적 함정 인지 + 정직성을
한 번에 보여준다.
